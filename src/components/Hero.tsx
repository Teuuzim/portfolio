...
export function Hero({ language }: HeroProps) {
  ...
  return (
    <section id="top" className="relative overflow-hidden pt-20">
      ...
      <div className="absolute -left-40 top-32 h-96 w-96 rounded-full bg-green-600/10 blur-3xl dark:bg-green-500/10" aria-hidden="true" />
      <div className="absolute -right-40 bottom-10 h-96 w-96 rounded-full bg-green-700/10 blur-3xl dark:bg-green-400/10" aria-hidden="true" />
      ...
      <div className="container-shell relative grid min-h-[calc(100vh-5rem)] items-center gap-16 py-20 lg:grid-cols-[1fr_0.7fr] lg:py-24">
        <div className="max-w-4xl">
          <div className="mb-7 inline-flex items-center gap-2 rounded-full border border-brand-700/15 bg-white/70 px-4 py-2 text-xs font-semibold uppercase tracking-[0.14em] text-brand-700 shadow-sm backdrop-blur dark:border-green-400/15 dark:bg-[#121C16]/70 dark:text-green-400">
            <span className="h-2 w-2 animate-pulse rounded-full bg-green-500" />
            {t.hero.eyebrow}
          </div>
          <p className="mb-4 font-display text-lg font-bold text-brand-700 dark:text-green-400">
            Matheus Henrique Vaz Marques
          </p>
          <h1 className="max-w-4xl font-display text-4xl font-extrabold leading-[1.08] tracking-[-0.045em] text-gray-900 sm:text-5xl lg:text-6xl xl:text-7xl dark:text-slate-50">
            {t.hero.title}
          </h1>
          <p className="mt-7 max-w-3xl text-lg font-medium leading-8 text-gray-700 sm:text-xl dark:text-slate-300">
            {t.hero.description}
          </p>
          <p className="mt-4 max-w-3xl text-base leading-7 text-gray-500 sm:text-lg dark:text-slate-400">
            {t.hero.secondary}
          </p>
